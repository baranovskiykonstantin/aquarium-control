...
Rectangle {
    ...
    Flickable {
        ...
        Text {
            ...
            text: qsTr("<b>About</b><br/><br/>This application is designed to control the aquarium controller described at <a href=\"https://github.com/baranovskiykonstantin/aquarium\">https://github.com/baranovskiykonstantin/aquarium</a>. It lets you connect to the aquarium through a serial (COM) port, view its current state, and change date, time, heating, lighting, and display settings. On Windows, a Bluetooth RFCOMM aquarium controller appears as a COM port.")
                + "<br/><br/>"
                + qsTr("<b>Communication ports</b><br/><br/>When the app starts, it shows a list of serial (COM) ports available on this computer. If the list is empty, make sure the controller is connected and the port is available in the system. Cancel returns to the main window without connecting. On the connecting screen, Cancel closes the port and returns to the main window.")
                + "<br/><br/>"
                + qsTr("<b>Selecting a port</b><br/><br/>Each row shows the port name. Tap a row to connect. Cancel returns to the main window without connecting.")
                + "<br/><br/>"
